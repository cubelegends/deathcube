...
public class BlockBar {
    private final ArrayList<Block> cageBlocks = new ArrayList<>();

    private final ArrayList<Block> cubeBlocks = new ArrayList<>();
    ...
    public BlockBar(final Location loc, final Vector vector) {
        this.origin = loc;
        this.vector = vector;
    ...
    public void load(final CommandSender sender) {
        final Location start = this.origin;
        ...
        if (!this.cubeBlocks.isEmpty() && !this.cageBlocks.isEmpty()) {
            final Location var7 = this.origin.clone().add(this.vector.clone().multiply(bugCounter + 1));
