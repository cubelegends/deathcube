...
public class BlockBar {
  ...
  public void setCageBlock(Block b) {
    if (this.isSet) {
      Block newBlock = this.cageBlocks.get(random(0, this.cageBlocks.size() - 1));
      b.setTypeIdAndData(newBlock.getTypeId(), newBlock.getData(), true);
    } else {
      b.setTypeIdAndData(35, (byte)this.fixedBlocks[random(0, 2)], false);
    } 
  }
  
  public void setCubeBlock(Block b) {
    if (this.isSet) {
      Block newBlock = this.cubeBlocks.get(random(0, this.cubeBlocks.size() - 1));
      b.setTypeIdAndData(newBlock.getTypeId(), newBlock.getData(), true);
    } else {
      b.setTypeIdAndData(35, (byte)this.fixedBlocks[random(0, 2)], false);
    } 
